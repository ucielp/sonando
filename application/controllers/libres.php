		...
		{ //check to see if the user is logging in
			...
			{ //if the login is successful
				...
				if ($this->ion_auth->is_group($group_name)){ //if the login is successful for an user that it's not an admin user
					redirect('libres/preinscribir', 'refresh');
				} else{
					$this->session->set_flashdata('message', $this->ion_auth->messages());
					redirect('libres', 'refresh');
				}
			}
			...
			{ //if the login was un-successful
				...
				redirect('libres', 'refresh'); //use redirects instead of loading views for compatibility with MY_Controller libraries
			}
		}
		else
